...
#define EEPROM_UPDATE 0	/*!< Set to \b 1 when you want to write EEPROM, otherwise leave set to \b 0 */
...
#if (EEPROM_UPDATE == 1)
	/**
	 * \attention	Function should only run once; control it with the #define of \c EEPROM_UPDATE
...
//	OSCCAL = eeprom_read_byte((uint8_t*)OSCALSPEED);
	...
	// Set up LEDs
	DDRD |= _BV(PD4);		// as output (LED1)
	LED1_OFF;
	DDRD |= _BV(PD5);		// as output (LED2)
	LED2_OFF;
...
void loop() {
	...
	if (rx_flag == 1) {
		...
		for (int i = 0; i < rxn; i++) {
			// For each new character received
			char currentChar = RX_BUFF[i];

			if (VGATONIC_MODE == 7) {
				// 'Y' Set Position Cursor
				currentChar-= 32;
				if (currentChar < 39) {
